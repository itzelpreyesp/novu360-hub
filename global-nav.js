(function () {
  ...
  const ensureNavStyles = () => {
    ...
    style.textContent = `
      ...
      #mobile-bottom-nav { display: none; position: fixed; bottom: 0; left: 0; right: 0; height: 65px; background: rgba(0,0,0,0.96); border-top: 1px solid rgba(0,194,168,0.12); z-index: 9970; align-items: center; justify-content: space-around; padding: 0 4px; backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); }
      ...
      #main-sidebar a.nav-active, #nav-drawer a.nav-active {
        background: linear-gradient(135deg, #00C2A8, #009984) !important;
        color: #000 !important;
        font-weight: 700 !important;
      }
      #main-sidebar a.nav-active .material-symbols-outlined, #nav-drawer a.nav-active .material-symbols-outlined { color: #000 !important; }
      @media (min-width: 1024px) {
        #mobile-bottom-nav { display: none !important; }
        #nav-hamburger { display: none !important; }
        #nav-drawer, #nav-drawer-backdrop { display: none !important; }
        body.novu-hide-legacy-sidebar .novu-legacy-sidebar { display: none !important; }
      }
  ...
  const moduleFiles = new Set(links.map((item) => item.href.split('/').pop().toLowerCase()));
  ...
  const applyActiveState = () => {
    document.querySelectorAll('a[href]').forEach((anchor) => {
      ...
      try {
        ...
        file = (url.pathname.split('/').pop() || 'dashboard.html').toLowerCase();
      } catch (e) {
        return;
      }
      if (!moduleFiles.has(file)) return;
      const isActive = file === current;
      anchor.classList.toggle('nav-active', isActive);
      anchor.classList.toggle('active', isActive);
    });
  };

  const createDesktopSidebar = () => {
    const existingMainSidebar = document.getElementById('main-sidebar');
    if (existingMainSidebar) {
      document.body.classList.add('novu-has-global-sidebar');
      return false;
    }

    const legacySidebar = document.querySelector('aside#desktop-sidebar, aside.hidden.lg\\:flex, aside[class*="lg:flex"]');
    if (legacySidebar) {
      legacySidebar.classList.add('novu-legacy-sidebar');
      document.body.classList.add('novu-hide-legacy-sidebar');
    }
  ...
  const createMobileNav = () => {
    ['nav-hamburger', 'nav-drawer-backdrop', 'nav-drawer', 'mobile-bottom-nav'].forEach((id) => {
    ...
    const bottomItems = [
      ...
      { href: '/ventas.html', label: 'Ventas', icon: 'trending_up' },
      { href: '/prospector-ia.html', label: 'Prospector', icon: 'person_search' },
